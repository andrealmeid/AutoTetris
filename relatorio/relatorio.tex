...
Através da Figura 7, que representa a pontuação da população de redes jogando o Tetris simplificado com uma única peça 1x1 como na Figura 3, podemos perceber que o algoritmo evolutivo aplicado é efetivo na otimização dos pesos da rede, aumentando gradativamente a média de pontos da população até que há um \textit{breakthrough} por volta da iteração 50, na qual as redes começam a atingir o teto de linhas limpas por jogo, que no caso do experimento era 500, colocado para evitar que os jogos tendessem ao infinito.
...
\textbf{Figura 7:} \textit{Evolução da pontuação das populações durante sessenta iterações}
\end{center}

Utilizando a Figura 8, percebemos também como a partir do \textit{breakthrough}, mesmo com uma rede conseguindo sempre as 500 linhas máximos e mantendo uma pontuação em torno dos 67000 pontos, a evolução continua já que a média continua a subir, mostrando que mais e mais redes da população estão conseguindo a pontuação máxima, cada uma utilizando uma estratégia diferente da outra.

\begin{center}
\includegraphics[scale=0.55]{tetris_graph1.png}\\

\textbf{Figura 8:} \textit{Pontuação média das populações durante sessenta iterações}
\end{center}
